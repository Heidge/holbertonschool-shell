...
I started 7-highlander in Terminal #0 and then run 6-stop_me_if_you_can in terminal #1, for every iteration we can see I am invincible!!! appearing in terminal #0.
...
```
sylvain@ubuntu$ sudo ./10-process_and_pid_file
To infinity and beyond
To infinity and beyond
^CY U no love me?!
```
...
```
sylvain@ubuntu$ sudo pkill -f 10-process_and_pid_file
sylvain@ubuntu$ 
```

Starting 10-process_and_pid_file in the terminal #0 and then killing it in the terminal #1.
...
I am alive!
^C
sylvain@ubuntu$ sudo ./11-manage_my_process stop
...
sylvain@ubuntu$ cat /var/run/my_process.pid 
11864
sylvain@ubuntu$ sudo ./11-manage_my_process restart
